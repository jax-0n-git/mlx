...
class MLX_API Scheduler {
 public:
  ...
  void enqueue_event(Stream s, Event event, std::function<void(Event&)> task);
  void set_error(Stream s, std::shared_ptr<std::string> error);
 ...
 private:
  ...
  std::unordered_map<int, std::list<Event>> events_;
...
template <typename F>
inline void enqueue_event(Stream s, Event event, F&& f) {
  scheduler().enqueue_event(s, std::move(event), std::forward<F>(f));
}
